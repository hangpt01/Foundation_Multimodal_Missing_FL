...
class Server(BasicServer):
    def __init__(self, option, model, clients, test_data = None):
        ...
        self.combin_cnt = {
            "image+sound": {
                "n_clients": sum([1 for client in self.clients if client.modal_combin == "image+sound"]),
                "n_data": sum([client.datavol for client in self.clients if client.modal_combin == "image+sound"])
            },
            "sound": {
                "n_clients": sum([1 for client in self.clients if client.modal_combin == "sound"]),
                "n_data": sum([client.datavol for client in self.clients if client.modal_combin == "sound"])
            }
        }
    ...
    def aggregate(self, models: list, modal_combins: list):
        ...
        new_model.sound_encoder = fmodule._model_sum([model_k.sound_encoder * pk for model_k, pk in zip(models, p)])
        new_model.sound_classifier = fmodule._model_sum([model_k.sound_classifier * pk for model_k, pk in zip(models, p)])
        if self.combin_cnt["image+sound"]["n_clients"] > 0:
            p = list()
            chosen_models = list()
            for cid, model, combin in zip(self.received_clients, models, modal_combins):
                if combin == "image+sound":
