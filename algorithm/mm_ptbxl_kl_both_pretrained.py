...
class Server(BasicServer):
    def __init__(self, option, model, clients, test_data = None):
        ...
        self.model.branchallleads.load_state_dict(torch.load(os.path.join(self.checkpoints_dir, 'full-modal', 'feature_extractor.pt')))
        self.model.branchallleads_classifier.load_state_dict(torch.load(os.path.join(self.checkpoints_dir, 'full-modal', 'classifier.pt')))
        self.model.branch2leads.load_state_dict(torch.load(os.path.join(self.checkpoints_dir, 'missing-modal', 'feature_extractor.pt')))
        self.model.branch2leads_classifier.load_state_dict(torch.load(os.path.join(self.checkpoints_dir, 'missing-modal', 'classifier.pt')))
