...
class Server(BasicServer):
    ...
    def aggregate(self, models: list):
        ...
        for k in range(n_models):    
            models[k].z_M = new_model.z_M
            
            #newly added
            p_k[k] = models[k].delta_G / (models[k].delta_O**2 *2)
...
class Client(BasicClient):
    ...
    def train(self, model, transformer, text_embeddings, client_id):
        ...
        val_loss_step_0 = torch.stack(self.evaluate(model, transformer, text_embeddings))
        # import pdb; pdb.set_trace()
    ...
    @fmodule.with_multi_gpus
    def evaluate(self, model, transformer, text_embeddings):
        """
        ...
        return self.calculator.evaluate(
            ...
            transformer=transformer,
            text_embeddings=text_embeddings,
            dataset=dataset   # list of batch mean eval loss of each leads
        )
