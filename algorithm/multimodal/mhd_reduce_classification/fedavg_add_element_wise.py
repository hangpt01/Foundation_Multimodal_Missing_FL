    ...
    def aggregate(self, models: list, names: list):
        ...
        for modality in self.modalities:
            ...
            if p.sum() == 0:
                continue
            p = p / p.sum()
            new_model.feature_extractors[modality] = fmodule._model_sum([
                model.feature_extractors[modality] * pk for model, pk in zip(models, p)
            ])
        p = np.array([self.count[name]['datavol'] for name in names])
        if p.sum() > 0:
            p = p / p.sum()
