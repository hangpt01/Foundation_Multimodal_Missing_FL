    ...
    def train(self, model, regularizer_model, transformer, text_embeddings, client_id):
        ...
        for iter in range(self.num_steps):
            ...
            if self.fedmsplit_prox_lambda > 0.0:
                for parameter, agg_parameter in zip(model.pooler.parameters(), regularizer_model.pooler.parameters()):
                    regular_loss += torch.sum(torch.pow(parameter - agg_parameter, 2))
                for parameter, agg_parameter in zip(model.classifier.parameters(), regularizer_model.classifier.parameters()):
                    regular_loss += torch.sum(torch.pow(parameter - agg_parameter, 2))
