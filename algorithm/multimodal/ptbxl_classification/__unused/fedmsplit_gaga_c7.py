...
class Server(BasicServer):
    ...
    def aggregate(self, models: list, modalities_list: list):
        ...
        global_attention_layer = fmodule._model_average([
                self.clients[self.selected_clients[k]].local_model.attention_layer for k in self.selected_clients
        ])
        for m in range(self.n_leads):
            for k in modal_dict[m]:
        ...
        for k in range(n_models):    
            ...
            self.clients[self.selected_clients[k]].local_model.attention_layer = global_attention_layer
        for k in range(n_models):
        ...
        new_model.attention_layer = global_attention_layer
