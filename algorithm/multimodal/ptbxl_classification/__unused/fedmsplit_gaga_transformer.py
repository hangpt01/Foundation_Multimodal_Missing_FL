...
class Server(BasicServer):
    ...
    def aggregate(self, models: list, modalities_list: list):
        ...
        global_transformer_encoder = fmodule._model_average(
            self.clients[self.selected_clients[k]].local_model.transformer_encoder for k in self.selected_clients
            )
        for k in range(n_models):    
            for m in range(self.n_leads):
                self.clients[self.selected_clients[k]].local_model.relation_embedders[m] = global_relation_embedders[m]
            self.clients[self.selected_clients[k]].local_model.transformer_encoder = global_transformer_encoder
        for k in range(n_models):
        ...
        new_model.transformer_encoder = global_transformer_encoder
