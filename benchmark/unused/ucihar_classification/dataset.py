...
class UCIHARDataset(Dataset):
    def __init__(self, root, download=True, standard_scaler=False, train=True, crop_length=0):
        ...
        if not os.path.exists(self.root):
            if download:
                print('Downloading UCIHAR Dataset...', end=' ')
                # os.makedirs(root, exist_ok=True)
                os.system('bash ./benchmark/ucihar_classification/download.sh')
                print('done!')
            else:
...
if __name__ == '__main__':
    dataset = UCIHARDataset(root='./benchmark/RAW_DATA/UCIHAR', standard_scaler=False, train=True)