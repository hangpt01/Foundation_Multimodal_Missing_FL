...
class MultimodalClassificationCalculator(ClassificationCalculator):
    def __init__(self, device, optimizer_name='sgd'):
        super(MultimodalClassificationCalculator, self).__init__(device, optimizer_name)
    ...
    @torch.no_grad()
    def custom_test(self, model, dataset, batch_size=64, num_workers=0, modal_combin=[], save_dir="", round_id=0):
        """
        Metric = [mean_accuracy, mean_loss]
        :param model:
        :param dataset:
        ...
        :return: [mean_accuracy, mean_loss]
        """
        y_pred_dict = {combin: list() for combin in modal_combin}
        loss_dict = {combin: 0.0 for combin in modal_combin}
        model.eval()
        if batch_size==-1:batch_size=len(dataset)
        ...
        for batch_id, batch_data in enumerate(data_loader):
            batch_data = self.data_to_device(batch_data)
            for i, combin in enumerate(modal_combin):
                outputs = model({
                    modal: batch_data[0][modal] for modal in combin.split("+")
                })
                y_pred = outputs.data.max(1, keepdim=True)[1]
                y_pred_dict[combin].extend(y_pred.squeeze().long().cpu().tolist())
                batch_mean_loss = self.criterion(outputs, batch_data[-1]).item()
                loss_dict[combin] += batch_mean_loss * len(batch_data[-1])
                if i == len(modal_combin) - 1:
                    correct = y_pred.eq(batch_data[-1].data.view_as(y_pred)).long().cpu().sum()
                    num_correct += correct.item()
                    total_loss += batch_mean_loss * len(batch_data[-1])
        for combin, y_pred in y_pred_dict.items():
            os.makedirs(os.path.join(save_dir, combin), exist_ok=True)
            np.save(os.path.join(save_dir, combin, 'Round{}.npy'.format(round_id)), np.array(y_pred))
            with open(os.path.join(save_dir, combin, 'loss.txt'), 'a') as f:
                f.write('{}\n'.format(loss_dict[combin] / len(dataset)))
        return {'accuracy': 1.0*num_correct/len(dataset), 'loss':total_loss/len(dataset)}

# =====================================TaskPipe======================================================
# This module is to save and load the fedtask that is generated by Generator. The target is to load the fedtask into a
