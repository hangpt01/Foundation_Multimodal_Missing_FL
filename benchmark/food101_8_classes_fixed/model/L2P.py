...
class Model(FModule):
    ...
    def infer(self, batch, transformer, text_embeddings):
        ...
        return cls_feats, batched_prompt, reduce_sim
    ...
    def forward(self, transformer, text_embeddings, batch, flag="", client_id=None, current_round=None):
        ...
        cls_feats, batched_prompt, reduce_sim = self.infer(batch, transformer, text_embeddings)
        ...
        if save_file:
            if flag != "":
                if flag=="train" and current_round % 25 == 0:
                    os.makedirs(f"output/food101/L2P/train/client_{client_id+1}/", exist_ok=True)
                    file_path = f"output/food101/L2P/train/client_{client_id+1}/sample_data_round_{current_round}.pt"
                    ...
                    os.makedirs("output/food101/L2P/test/", exist_ok=True)
                    file_path = f"output/food101/L2P/test/sample_data_round_{current_round}.pt"
                    if os.path.exists(file_path):
                        # Load existing data
                        existing_data = torch.load(file_path)
                        if isinstance(existing_data, list):
