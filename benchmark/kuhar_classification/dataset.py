...
class KUHARDataset(Dataset):
    def __init__(self, root, download=True, standard_scaler=False, train=True, crop_length=0):
        ...
        if not os.path.exists(self.root):
            if download:
                print('Downloading KUHAR Dataset...', end=' ')
                os.makedirs(root, exist_ok=True)
                os.system('bash ./benchmark/kuhar_classification/download.sh')
                print('done!')
            else:
        ...
        #     test_vids = pickle.load(open(os.path.join(self.root, 'KUHAR_features.pkl'),"rb"), encoding='latin1')
...
if __name__ == '__main__':
    dataset = KUHARDataset(root='./benchmark/RAW_DATA/KUHAR', standard_scaler=False, train=True)