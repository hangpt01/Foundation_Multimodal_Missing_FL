...
class Model(FModule):
    ...
    def infer(
        ...
        for i, blk in enumerate(self.transformer.blocks):
            ...
            if i in self.prompt_layers:
                if self.multi_layer_prompt:
                    ...
                    import pdb; pdb.set_trace()
                    x, _attn = blk(x, mask=co_masks,                                    # 500MB each loop: x:(batch, dim, 768) -> (batch, dim+16, 768)
                ...
                x, _attn = blk(x, mask=co_masks)        # only this line: 2560MB, 88,81,768
        ...
        #         if i in self.prompt_layers:
        #             if self.multi_layer_prompt:
        #                 x, _ = blk(x, mask=co_masks,
        #                         prompts=prompts[:, self.prompt_layers.index(i)],
        #                         learnt_p=self.learnt_p,
        #                         prompt_type=self.prompt_type)
        #             else:
        #                 x, _ = blk(x, mask=co_masks, prompts=prompts, learnt_p=self.learnt_p)
        #         else:
        #             x, _ = blk(x, mask=co_masks)
