...
class Model(FModule):
    def __init__(self, pool_size=10, top_k=5, num_classes=10):
        ...
        for i in range(len(self.hparams_config["prompt_layers"])):
            self.pools.append(Pool(embed_dim=768, pool_size=pool_size, top_k=top_k))
        self.pooler = Pooler()
        ...
        for i in range(len(self.hparams_config["prompt_layers"])):
            self.trained_prompts_checklist = torch.zeros(self.pools[i].prompt.shape[0], dtype=torch.float32)
    ...
    def infer(self, batch, transformer, text_embeddings):
        ...
        for i, blk in enumerate(self.transformer.blocks):
            if i in self.hparams_config["prompt_layers"]:
                reduce_sim, batched_prompt = self.pools[i](x, cls_features=None)
                # print("Prompt in {} layer".format(i+1), torch.sum(batched_prompt))
                prompt_masks = torch.ones(batched_prompt.shape[0], batched_prompt.shape[1], dtype=batched_prompt.dtype, device=batched_prompt.device).long()
                co_masks = torch.cat([prompt_masks, text_masks, image_masks], dim=1)    # torch.Size([batch, 329]);     batch, 353=257+96
    ...
    def checking_trained_prompt(self):
        for i in range(len(self.hparams_config["prompt_layers"])):
            if self.pools[i].top_k_idx.device != self.trained_prompts_checklist.device:
                self.trained_prompts_checklist = self.trained_prompts_checklist.to(self.pools[i].top_k_idx.device)
            self.trained_prompts_checklist[self.pools[i].top_k_idx] += 1.0
