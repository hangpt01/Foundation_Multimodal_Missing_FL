...
class Model(FModule):
    def __init__(self):
        ...
        zero_prompt = torch.zeros(prompt_num, prompt_length, embed_dim)
        self.zero_prompt = nn.Parameter(zero_prompt)


    def only_image(self, batch, index):
        return

    def logits_missing_image(       # text_only
            self,
            batch,
            transformer,
            text_embeddings,
        ...
        img = batch[imgkey][0]     # ([64, 3, 608, 608])
        ...
        for idx in range(len(img)):
            if batch["missing_type"][idx] == 0:         # complete -> turn img into dummy
                prompt = self.missing_img_prompt
                sample_img = img[idx, :, :, :]
                img[idx, :, :, :] = torch.ones(sample_img.size()).float() 
            elif batch["missing_type"][idx] == 1:       # missing_text
                prompt = self.zero_prompt
        ...
        # import pdb; pdb.set_trace()
        # (batch, 40, 768), (batch, 217 (or 41), 768)
...
#         cls_feats = self.pooler(x[:,:prompts.size(1)].mean(dim=1,keepdim=True))
    ...
    def infer(
        ...
        logits_missing_img = self.logits_missing_image(batch,
            transformer,
            text_embeddings,
            mask_text=False,
            mask_image=False,
