...
class Model(FModule):
    def __init__(self, pool_size=10, top_k=5, num_classes=10):
        ...
        # Pool and Pooler
        self.pool = Pool(embed_dim=768, pool_size=pool_size, top_k=6)
        self.pooler = Pooler()
    ...
    def infer(self, batch, transformer, text_embeddings):
        ...
        # attention-level
        prompt_masks = torch.ones(batched_prompt.shape[0], batched_prompt.shape[1]//2, dtype=batched_prompt.dtype, device=batched_prompt.device).long()
        ...
        for i, blk in enumerate(self.transformer.blocks):
            # x, _attn = blk(x, mask=co_masks)
            if i == 0:
                x, _attn = blk(x, mask=co_masks, prompts=batched_prompt, learnt_p=True, prompt_type=self.hparams_config['prompt_type'])
            else:
                x, _attn = blk(x, mask=co_masks)
