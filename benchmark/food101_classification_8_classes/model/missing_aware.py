        ...
        if save_file:
            if flag != "":
                if flag=="train" and current_round % 10 == 0:
                    os.makedirs(f"output/food101/missing_aware/train/client_{client_id+1}/", exist_ok=True)
                    file_path = f"output/food101/missing_aware/train/client_{client_id+1}/sample_data_round_{current_round}.pt"
                    ...
                    os.makedirs("output/food101/missing_aware/test/", exist_ok=True)
                    file_path = f"output/food101/missing_aware/test/sample_data_round_{current_round}.pt"
                    if os.path.exists(file_path):
                        # Load existing data
                        existing_data = torch.load(file_path)
                        if isinstance(existing_data, list):
