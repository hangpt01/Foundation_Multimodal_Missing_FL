...
class AttentionLayer(FModule):
    ...
    def forward(self, embeddings, relation_infos, device):       #target, neighbor: (batch,128) - (128,128)
        batch_size = embeddings.shape[0]
        embedding_outputs = torch.zeros(size=(batch_size, 128), dtype=torch.float32, device=device)
        relation_info_outputs = torch.zeros(size=(batch_size, 128), dtype=torch.float32, device=device)
        for i in range(batch_size):    
            h = torch.zeros(size=(2, 128), dtype=torch.float32, device=device)
            h[0,:] = embeddings[i,:]
            h[1,:] = relation_infos[i,:]
...
class Classifier(FModule):
    def __init__(self):
        ...
        self.ln1 = nn.Linear(128*2, 128, True)
...
class Model(FModule):
    ...
    def forward(self, x, y, leads):
        ...
        relation_infos_sum = torch.zeros(size=(batch_size, 128), dtype=torch.float32, device=y.device)
        for lead in total_lead_ind:    
            if lead in leads:
                ...
                relation_infos[:,lead*128:(lead+1)*128] = relation_info
                relation_infos_sum += relation_info
                self.relation_embedders[lead].relation_embedder.weight.data[0].zero_()
            else:
                relation_info = self.relation_embedders[lead](y.device, has_modal=False).repeat(batch_size,1)        # 128, 256
                relation_infos[:,lead*128:(lead+1)*128] = relation_info
                relation_infos_sum += relation_info
                self.relation_embedders[lead].relation_embedder.weight.data[1].zero_()

        features_prime, relation_info_prime = self.attention_layer(features,relation_infos_sum, y.device)
        # import pdb; pdb.set_trace()
