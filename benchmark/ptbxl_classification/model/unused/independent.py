...
class LeadSpecific(FModule):
    def __init__(self):
        super(LeadSpecific, self).__init__()
...
class Model(FModule):
    def __init__(self):
        ...
        self.n_leads = 12
        self.lead_specific = nn.ModuleList()
        for i in range(self.n_leads):
            self.lead_specific.append(LeadSpecific())

    def forward(self, x, y, leads):
        batch_size = y.shape[0]
        loss = 0.0
        for lead in leads:
            loss += self.lead_specific[lead](x[:, lead, :].view(batch_size, 1, -1), y)[0]
        return loss, None
    
    def predict(self, x, y, leads):
        batch_size = y.shape[0]
        predict = torch.zeros(size=(batch_size, 10), dtype=torch.float32, device=x.device)
        for lead in leads:
